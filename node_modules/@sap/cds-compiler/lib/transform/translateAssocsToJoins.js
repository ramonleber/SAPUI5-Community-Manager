  ...
  function rewriteGenericPaths(pathNode, env)
  {
    ...
    else
    {
      ...
      if(QA) {
        ...
        // if(!(tail[tail.length-1]._artifact._effectiveType.elements ||
        //      tail[tail.length-1]._artifact._effectiveType.target)) {
    ...
    function rightMostQA(path) {
      ...
      let ps = path[pl--];
      // TODO discuss: what is this _parent supposed to be? it is the query...
      // I thought $QA is on the table alias...
          ...
          else if(i < args.length-2 && args[i].path && args[i+1] === '=' && args[i+2].path)
          {
            ...
            if(fwdAssoc)
            {
              ...
              if(env.assocStack.length == 2) {
                ...
                error(null, env.assocStack[0].location, `An association that uses "$self" in its ON-condition can't be compared to "$self"`);
              ...
              */
              ...
              path.forEach(ps => { // eslint-disable-line consistent-return
                if(ps._artifact.target) {
                  error(null, pathNode.location, `"${env.lead.name.absolute}": "${ps.id}" in path "${pathAsStr(pathNode.path)}" must not be an association`);
  ...
  */
  function checkPathDictionary(pathDict, env)
  {
    ...
    path.forEach((ps) => {
      ...
      if(ps.args) {
        error(null, pathDict.location, `"${lead.name.absolute}": "${ps.id}" must not have parameters`);
        pathDict.$check = false;
      }
      if(ps.where) {
        error(null, pathDict.location, `"${lead.name.absolute}": "${ps.id}" must not have a filter`);
        pathDict.$check = false;
      }
      if(ps._artifact.virtual) {
        error(null, pathDict.location, `"${lead.name.absolute}": "${ps.id}" must not be virtual`);
        pathDict.$check = false;
      }
      ...
      if(ps._artifact && ps._artifact.target)
      {
        ...
        if(la1) {
          if(ps._artifact.on)
          {
            error(null, pathDict.location, `"${lead.name.absolute}": "${ps.id}" in path "${pathAsStr(pathDict.path)}" must not be an unmanaged association`);
            pathDict.$check = false;
          }
          else if(ps._artifact.$fkPathPrefixTree)// must be managed
          {
            if(!ps._artifact.$fkPathPrefixTree.children[la1.id]) {
              error(null, pathDict.location, `"${lead.name.absolute}": "${la1.id}" is not foreign key of managed association "${ps.id}" in path "${pathAsStr(pathDict.path)}"` );
              pathDict.$check = false;
            }
          }
        }
          ...
          error(null, pathDict.location, `"${lead.name.absolute}": "${ps.id}" in path "${pathAsStr(pathDict.path)}" must not be an association`);
          pathDict.$check = false;
        }
      }
    });
    ...
    if (artifact && artifact.elements) {
      error(null, pathDict.location, `"${lead.name.absolute}": "${lastSegment.id}" must have scalar type`);
      pathDict.$check = false;
    }
    return pathDict.$check;
  }
