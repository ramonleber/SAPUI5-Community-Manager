...
function toHdbcdsSource(csn, options) {
  ...
  const plainNames = options.forHana && options.forHana.names === 'plain';
  const hdbcdsNames = options.forHana && options.forHana.names === 'hdbcds';
  ...
  const killList = [];
  if (options.toHana && options.toHana.names === 'quoted')
    addContextMarkers(csn, killList);

  if (options.toHana && options.toHana.names !== 'plain')
  ...
  for (let artifactName in getTopLevelArtifacts()) {
    ...
    if (sourceStr !== '') {
      artifactName = plainNames ? artifactName.replace(/\./g, '_').toUpperCase() : artifactName;
      hdbcds[artifactName] = `${options.testMode ? ''
        : `// ${
          generatedByCompilerVersion()
        } \n`
      }${
        renderNamespaceDeclaration(artifactName, env)
      }${
        renderUsings(artifactName, env)
      }${
        sourceStr
      }`;
    }
   ...
   *
   * @param {CSN.Extension[]} extensions Extensions to render
   * @param {CdlRenderEnvironment} env Environment
   * @returns {string} Rendered extensions
   */
        ...
        for (const name in ext.actions) {
          ...
          if (action.params) {
            ...
            for (const paramName in action.params)
              paramAnnotations.push(grandChildEnv.indent + quoteId(paramName));
   ...
   *
   * @param {CSN.Elements} elements Element extension to render
   * @param {CdlRenderEnvironment} env Environment
   * @returns {string} Rendered element extensions
   */
  ...
  function renderArtifact(artifactName, art, env) {
    ...
    switch (art.kind) {
      ...
      case 'type':
      case 'aspect':
        return renderType(artifactName, art, env, art.$syntax);
      case 'annotation':
      case 'action':
      case 'function':
      case 'event':
...
*/
  ...
  function renderEntity(artifactName, art, env) {
    ...
    if (globalDuplicateChecker)
      globalDuplicateChecker.addArtifact(normalizedArtifactName, art && art.$location, artifactName);
   ...
   */
  function renderType(artifactName, art, env, artType) {
    if (!options.toCdl && art.kind === 'aspect' || art.kind === 'type' && options.toHana.names !== 'hdbcds' || art.kind === 'type' && options.toHana.names === 'hdbcds' && !art.elements)
      return '';
    let result = '';
    result += `${env.indent + (options.toCdl && (artType || art.$syntax) || art.kind )} ${renderArtifactName(artifactName, env, true)}`;
    if (art.includes) {
      // Includes are never flattened (don't exist in HANA)
      result += ` : ${art.includes.map(name => renderAbsoluteNameWithQuotes(name, env)).join(', ')}`;
    }
  ...
  function renderTypeReference(elm, env, noEnum = false) {
    ...
    if (!elm.type) {
      ...
      const dontRenderKeyForNestedElement = options.toHana && options.toHana.names === 'hdbcds';
