...
class JoinCQNFromExpanded {
  ...
  _isDraftTree(table) {
    ...
    if (!this._isDraft) {
      ...
      if (this._isDraft && !this._draftService) {
        const entity =
          this._csn.definitions[ensureUnlocalized(table)] || this._csn.definitions[table.replace(/_drafts$/i, '')]
        this._draftService = entity._service.name
      }
    }
  ...
  _getJoinRef(elements, column, isActive, defaultLanguage) {
    ...
    // REVISIT: old, complex impl used this._draftService due to unknown reason... -> can be removed if no complaints
    // return this._draftService + '.' + assoc.target.replace(assoc._target._service.name + '.', '') + '_drafts'
